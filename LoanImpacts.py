...
class LoanImpacts:
    ...
    def __init__(self, principal, rate, term, extra_payment, contributions):
    ...
    def compute_impacts(self):
        ...
        loan_all = Loan(principal=self.principal, rate=self.rate,
                        term=self.term, extra_payment=self.extra_payment + sum(self.contributions))
        ...
        loan_none = Loan(principal=self.principal, rate=self.rate,
                         term=self.term, extra_payment=self.extra_payment)
        ...
        for index, contribution in enumerate(self.contributions):
            loan_index = Loan(principal=self.principal, rate=self.rate, term=self.term,
